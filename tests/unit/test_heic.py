...
class TestHEICHandler(unittest.TestCase):
    def setUp(self):
        self.handler = HEICHandler(config={'password': 'test_secret'})
        self.test_heic_path = r'tests\fixtures\test.heic'
    ...
    def test_embed_success(self):
        """测试HEIC水印嵌入成功"""
        output_path = os.path.join(self.temp_dir, 'test_embed.heic')
        watermark = WatermarkData(content='HEICTest-2026')
        
        result = self.handler.embed(
            file_path=self.test_heic_path,
            watermark=watermark,
    ...
    def test_verify_success(self):
        ...
        # 嵌入
        embed_result = self.handler.embed(self.test_heic_path, watermark, output_path)
