...
def ensure_dir(dir_path: str) -> None:
    ...
    Note:
        ...
        不会在目录已存在时抛出异常。
    """
    logger.debug(f"Ensuring directory: {dir_path}")
...
def get_file_size(file_path: str) -> int:
    ...
    """
    logger.debug(f"Getting file size: {file_path}")
...
def list_files(directory: str, extensions: Optional[List[str]] = None) -> List[str]:
    ...
    """
    logger.info(f"Listing files in: {directory}, extensions: {extensions}")
...
def human_readable_size(size_bytes: int) -> str:
    ...
    Example:
        ...
        '1.46 MB'
    """
    logger.debug(f"Converting size: {size_bytes} bytes")
